...
/**
 * PDF 파일에서 텍스트를 추출하고 일정 정보를 파싱합니다.
 */
export async function parsePdfPlan(buffer) {
  try {
    // PDF에서 텍스트 추출
    const data = await pdf(buffer);
    const text = data.text;
    
    console.log('PDF 텍스트 추출 완료:', text.substring(0, 500) + '...');
    ...
    const patterns = {
      ...
      // 장소 패턴 (주요 도시명, 관광지명)
      location: /(서울|부산|대구|인천|광주|대전|울산|세종|수원|고양|용인|성남|청주|전주|천안|안산|안양|부천|남양주|화성|평택|의정부|시흥|파주|광명|김포|군포|이천|오산|하남|양주|구리|안성|포천|의왕|여주|동두천|과천|가평|연천|제주|서귀포|[가-힣]+(?:궁|탑|공원|박물관|미술관|전시관|체험관|놀이공원|리조트|호텔|식당|카페|마켓|마트|백화점|쇼핑몰|역|공항|터미널|항구|해변|산|강|호수|다리|광장|거리|시장))/g,
      
      // 활동 패턴
      activity: /(견학|체험|관람|투어|쇼핑|식사|휴식|자유시간|이동|출발|도착|집합|해산|문화|역사|자연|전통|현대|산업|교육|엔터테인먼트|레저|스포츠)/g
    };
    
    // 일정 정보 추출
    const schedules = extractScheduleInfo(text, patterns);
 ...
 * 텍스트에서 일정 정보를 추출합니다.
 */
function extractScheduleInfo(text, patterns) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (timeMatches || locationMatches || activityMatches) {
      const schedule = {
        ...
        location: locationMatches ? locationMatches[0] : '미정',
        activityName: extractActivityName(line),
        type: determineActivityType(line),
        autoGenerated: true,
        adminNotes: `PDF에서 자동 추출: "${line}"`
      };
  ...
  return [
    {
      ...
      type: 'activity',
      autoGenerated: true,
      adminNotes: 'PDF 파싱 결과 - 기본 일정 생성됨'
    },
    {
      id: 'default_2',
      date: generateDefaultDate(1),
...
function validateAndCleanSchedules(schedules) {
  return schedules.map((schedule, index) => ({
    ...schedule,
    id: schedule.id || `parsed_${index + 1}`,
...
export function validatePdfFile(file) {
  ...
  // 최소 크기 검사 (1KB 이상)
  if (file.size < 1024) {
    errors.push('파일이 너무 작습니다. 올바른 PDF 파일인지 확인해주세요.');
  }
