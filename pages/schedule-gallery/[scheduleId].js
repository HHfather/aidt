...
export default function ScheduleGallery() {
  ...
  const loadScheduleInfo = async () => {
    try {
      ...
      if (data.success && data.schedule) {
        setScheduleInfo({
          title: data.schedule.location || data.schedule.activity || '연수 일정',
  ...
  const loadPhotos = async () => {
    try {
      setLoading(true)
      const response = await fetch(`/api/schedule-gallery?scheduleId=${scheduleId}`)
      const data = await response.json()
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      formData.append('scheduleTitle', scheduleInfo?.title || '연수 일정')
      formData.append('scheduleDate', scheduleInfo?.date || new Date().toISOString().split('T')[0])
      formData.append('description', description)

      const response = await fetch('/api/schedule-gallery', {
        method: 'POST',
        body: formData
      })
  ...
  const handleDelete = async (photoId) => {
    ...
    try {
      const response = await fetch(`/api/schedule-gallery?scheduleId=${scheduleId}&photoId=${photoId}`, {
        method: 'DELETE'
      })
