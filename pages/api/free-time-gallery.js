  ...
  return new Promise(async (resolve, reject) => {
    if (req.method === 'GET') {
      try {
        ...
        console.log('Free time gallery GET request for scheduleId:', scheduleId);
        ...
        const q = query(
          collection(db, 'free-time-gallery'),
        ...
        console.log('Found free time gallery images:', images.length);
        res.status(200).json({ success: true, photos: images });
      } catch (error) {
        console.error('자유시간 갤러리 조회 오류:', error);
        res.status(500).json({ success: false, error: '자유시간 갤러리 조회 중 오류가 발생했습니다.' });
      }
      resolve();
      ...
      form.parse(req, async (err, fields, files) => {
        ...
        try {
          ...
          const dayNumber = getField(fields, 'dayNumber'); // 몇 번째 날인지
          ...
          const storageRef = ref(storage, `free-time-gallery/${uniqueFileName}`);
          ...
          const docRef = await addDoc(collection(db, "free-time-gallery"), newImageData);
      ...
      try {
        ...
        const docRef = doc(db, 'free-time-gallery', id);
        ...
        if (imageData.fileName) {
          try {
            const storageRef = ref(storage, `free-time-gallery/${imageData.fileName}`);
