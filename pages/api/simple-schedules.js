...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    try {
      console.log('간단한 일정 조회 요청');

      let allSchedules = [];

      // 1. schedules 컬렉션에서 일정 가져오기 (단순 쿼리)
      try {
      ...
      console.log(`간단한 일정 조회 완료: ${processedSchedules.length}개 일정, ${Object.keys(schedulesByRegion).length}개 권역`);
      ...
      res.status(200).json({
        ...
        allSchedulesByDate: schedulesByDate
      });

    } catch (error) {
      console.error('간단한 일정 조회 오류:', error);
